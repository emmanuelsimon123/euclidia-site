...
It will ask you questions, update products.json, copy your thumbnail,
...
def load_products():
    ...
    with open(PRODUCTS_FILE, 'r', encoding='utf-8') as f:
        try:
            ...
            print_error("products.json has a formatting error. Starting fresh.")
            return []

def save_products(products):
    with open(PRODUCTS_FILE, 'w', encoding='utf-8') as f:
        json.dump(products, f, indent=2, ensure_ascii=False)
    print_success(f"products.json updated ({len(products)} product{'s' if len(products) != 1 else ''} total)")

def git_push(files, commit_message):
    try:
        subprocess.run(['git', 'config', '--global', 'user.email', 'euclidiamath@gmail.com'], capture_output=True)
        subprocess.run(['git', 'config', '--global', 'user.name', 'Euclidia'], capture_output=True)
...
def main():
    ...
    if not os.path.exists('shop.html'):
    ...
    # Load existing products, add new one, save
    ...
    if git_push(files_to_push, f"Add product: {data['title']}"):
        print()
        print(f"{GREEN}{BOLD}{'='*50}{RESET}")
        print(f"{GREEN}{BOLD}   Product Added Successfully!{RESET}")
        print(f"{GREEN}{BOLD}{'='*50}{RESET}")
        print(f"{GREEN}✓ products.json updated ({len(products)} products total){RESET}")
        if data['thumbnail']:
        ...
        print_info("products.json was updated locally but not pushed.")
